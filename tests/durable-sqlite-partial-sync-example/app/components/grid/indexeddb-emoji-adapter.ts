...
import type { EmojiGridRowVal } from "./emoji-grid-row-schema";

const INDEXEDDB_STORE_NAME = "emoji_grid";
...
export function createEmojiGridIndexedDbAdapter(roomId: string): {
	adapter: KeyValAdapter<EmojiGridRowVal>;
	readyPromise: Promise<void>;
} {
	const dbName = `durable-partial-sync-emoji-${roomId}`;
	...
	const adapter: KeyValAdapter<EmojiGridRowVal> = {
		get: async (key) => {
		...
		entries: async () => {
			...
			return keys.map((key, index) => [
				String(key),
				values[index] as EmojiGridRowVal,
			]);
		},
		clear: async () => {
			const db = await dbPromise;
