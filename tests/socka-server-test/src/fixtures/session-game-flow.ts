...
export async function assertSessionGameRosterGrowth(
	rpcA: SockaSession<typeof sessionGameContract>,
	createRpcB: () =>
		| SockaSession<typeof sessionGameContract>
		| Promise<SockaSession<typeof sessionGameContract>>,
	expect: SessionGameExpectFn,
): Promise<SockaSession<typeof sessionGameContract>> {
	await rpcA.client.waitForOpen();

	const onlyA = await rpcA.send.listPlayers();
	...
	const aView = await rpcA.send.listPlayers();
	const bView = await rpcB.send.listPlayers();
...
export async function assertSessionGameCombat(
	rpcA: SockaSession<typeof sessionGameContract>,
	rpcB: SockaSession<typeof sessionGameContract>,
	expect: SessionGameExpectFn,
): Promise<void> {
	await rpcA.client.waitForOpen();
	await rpcB.client.waitForOpen();
...
/** Collect server push events for assertions (pass `handlers` into {@link SockaSession}). */
export function createSessionGameEventCollector(): {
	...
	handlers: Partial<InferSockaPushHandlers<typeof sessionGameContract>>;
} {
 ...
 * pass collector `pushHandlers` into {@link SockaSession}).
 */
export async function assertSessionGamePlayerJoinedEvent(
	rpcWitness: SockaSession<typeof sessionGameContract>,
	createRpcPeer: () =>
		| SockaSession<typeof sessionGameContract>
		| Promise<SockaSession<typeof sessionGameContract>>,
...
export async function assertSessionGameEliminationWitnessed(
	rpcAttacker: SockaSession<typeof sessionGameContract>,
	createRpcVictim: () =>
		| SockaSession<typeof sessionGameContract>
		| Promise<SockaSession<typeof sessionGameContract>>,
	createRpcWitness: () =>
	...
	rpcVictim: SockaSession<typeof sessionGameContract>;
	rpcWitness: SockaSession<typeof sessionGameContract>;
}> {
	await rpcAttacker.client.waitForOpen();
	const aView = await rpcAttacker.send.listPlayers();
	...
	const hit1 = await rpcAttacker.send.attack({ target: idB });
	expect(hit1.gameOver).toBe(false);

	const fin = await rpcAttacker.send.attack({ target: idB });
	expect(fin.gameOver).toBe(true);
