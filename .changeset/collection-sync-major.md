...
**Breaking:** Sync and partial-sync APIs require `PartialSyncRowShape` — every row must declare an `updatedAt` key (values may be `number`, `Date`, `null`, or `undefined` for “no watermark”).
