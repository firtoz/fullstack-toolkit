...
- **`socka/react`**: `useSockaRpc(contract, options, deps)` hook returning typed `rpc` object; `SockaRpcProvider` + `useSockaRpcContext(contract)` for a single shared connection in the React tree.
